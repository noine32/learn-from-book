...
🔎 **Live demo (showcase):** https://noine32.github.io/learn-from-book/
...
See [`docs/DEMO.md`](docs/DEMO.md) ([English](docs/DEMO.en.md)) for verified techniques across
all runtimes (Node/TypeScript, Python/pytest, Go, Rust, and Excel/VBA), each verified by execution.
...
- **Go runtime:** working — runs `go test` and mutates function bodies to `panic("MUTATED")`.
- **Rust runtime:** working — runs `cargo test` and mutates `pub fn` bodies to `panic!("MUTATED")`.
